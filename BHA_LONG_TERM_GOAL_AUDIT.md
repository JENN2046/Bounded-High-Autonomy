...
| V1 stable audit and exit status | `audit-v1-stable`, `stable-exit-status`, `stable-exit-review`, `BHA_V1_STABILITY.md`, `.bha/validation.yaml` | `node scripts/bha-run.js audit-v1-stable --format json`; `node scripts/bha-run.js stable-exit-status --remote 'origin' --branch 'master' --format json`; `node scripts/bha-run.js stable-exit-review --remote 'origin' --branch 'master' --format json` | stable candidate |
...
| V1 stable audit freezes hard boundaries, proof sources, validation coverage, and V1-only production capability scope | `audit-v1-stable`, `stable-exit-status`, `stable-exit-review`, `.bha/validation.yaml`, `.bha/policy.yaml` | `node scripts/bha-run.js audit-v1-stable --format json`; `node scripts/bha-run.js stable-exit-status --remote 'origin' --branch 'master' --format json`; `node scripts/bha-run.js stable-exit-review --remote 'origin' --branch 'master' --format json` | Stable exit status and stable exit review are read-only phase readiness; they do not replace `validate` or the verifier. |
