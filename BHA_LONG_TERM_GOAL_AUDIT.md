...
| Long-term completion remains explicitly incomplete until future V2 enablement has a new objective and evidence | `long-term-goal-status`, `next-local-plan-status`, `BHA_LONG_TERM_GOAL_AUDIT.md` | `node scripts/bha-run.js long-term-goal-status --remote 'origin' --branch 'master' --format json`; validation id `long_term_goal_status_readonly` | Current local state may be clean while `completion_boundary.long_term_goal_complete=false`. |
