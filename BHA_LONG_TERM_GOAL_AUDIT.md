...
| V1 stable audit freezes hard boundaries, proof sources, validation coverage, and V1-only production capability scope | `audit-v1-stable`, `.bha/validation.yaml`, `.bha/policy.yaml` | `node scripts/bha-run.js audit-v1-stable --format json` | Stable audit is an artifact coverage audit and does not replace `validate` or the verifier. |
